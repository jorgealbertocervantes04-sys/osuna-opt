...
export default function LoginApp() {
  const [pasoActual, setPasoActual] = useState('telefono');
      ...
      <div style={{ background: '#1e293b', padding: '40px 30px', borderRadius: '20px', boxShadow: '0 20px 40px rgba(0, 0, 0, 0.6)', width: '100%', maxWidth: '400px', boxSizing: 'border-box', border: '1px solid #334155' }}>
        ...
        {pasoActual === 'telefono' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <button onClick={verificarCelular} disabled={cargando} style={btnStyle}>{cargando ? 'Buscando...' : 'Verificar Celular'}</button>
        ...
        {pasoActual === 'registro' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <input type="text" name="generacion" placeholder="Generación (Ej. G122)" value={regData.generacion} onChange={(e) => setRegData({...regData, generacion: e.target.value})} style={inputStyle} />
            <input type="text" name="nombre" placeholder="Nombre Completo" value={regData.nombre} onChange={(e) => setRegData({...regData, nombre: e.target.value})} style={inputStyle} />
            <input type="text" name="empleado" placeholder="Número de Empleado" value={regData.empleado} onChange={(e) => setRegData({...regData, empleado: e.target.value})} style={inputStyle} />
            ...
            {!esInduccion && (
              <>
                <input type="text" name="empresa" placeholder="Empresa (Ej. Larmex)" value={regData.empresa} onChange={(e) => setRegData({...regData, empresa: e.target.value})} style={inputStyle} />
                <select name="unidad" value={regData.unidad} onChange={(e) => setRegData({...regData, unidad: e.target.value})} style={selectStyle}>
                  <option value="">Selecciona Unidad de Negocio...</option>
                  {catalogos.unidades.map((u, i) => <option key={i} value={u.nombre}>{u.nombre}</option>)}
                </select>
                <select name="lider" value={regData.lider} onChange={(e) => setRegData({...regData, lider: e.target.value})} style={selectStyle}>
                  <option value="">Selecciona tu Líder...</option>
                  {catalogos.lideres.map((l, i) => <option key={i} value={l.nombre}>{l.nombre}</option>)}
                </select>
                <select name="gerente" value={regData.gerente} onChange={(e) => setRegData({...regData, gerente: e.target.value})} style={selectStyle}>
                  <option value="">Selecciona tu Gerente...</option>
                  {catalogos.gerentes.map((g, i) => <option key={i} value={g.nombre}>{g.nombre}</option>)}
                </select>
                <select name="tutor" value={regData.tutor} onChange={(e) => setRegData({...regData, tutor: e.target.value})} style={selectStyle}>
                  <option value="">Selecciona tu Tutor (OPT)...</option>
                  {catalogos.tutores.map((t, i) => <option key={i} value={t.nombre}>{t.nombre}</option>)}
                </select>
              </>
            )}
            <button onClick={registrarUsuario} disabled={cargando} style={btnStyle}>{cargando ? 'Activando...' : 'Activar Mi Cuenta'}</button>
          </div>
        )}
        ...
        {pasoActual === 'password' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <p style={{ fontSize: '14px', color: '#0ea5e9', fontWeight: 700, marginBottom: '25px' }}>¡Bienvenido, {usuarioActual?.nombre_completo?.split(' ')[0] || 'Operador'}!</p>
